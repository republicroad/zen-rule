...
from .register import udf_manager
from .contrib import *
...
class ZenRule:
    ...
    def __init__(self, options: Optional[dict] = None) -> None: 
        # {"customHandler": self.custom_handler_func, "loader": self.loader}
        if options:
            ...
            if not options.get("customHandler"):
                options["customHandler"] = self.custom_handler_func
            self.options = options
        else: # custom_async_handler self.custom_handler
            self.options = {"customHandler": self.custom_handler_func}
            ...
            if node.get("type") == "customNode":
                ...
                if custom_expressions:
                    for func_item in custom_expressions:
                        item = {**func_item}
                        item["value"] = self.parse_oprator_expr(func_item["value"])
    ...
    @classmethod
    async def custom_handler_func(cls, request):
        """
        ...
        for item in expr_asts:
            # 未来自定义函数需要支持引用前面函数的返回值作为参数. 那么此处就要函数的执行顺序的拓扑依赖.
            coro_funcs.append(cls.engine(item, request.input, context))
        ...
        logger.debug("custom result: %s", results)
        return {
            "output": results
        }

    @classmethod
    async def engine(cls, exec_expr, node_input, context={}): 
        """
            item 代表自定义节点中的一个函数
    ...
    def parse_oprator_expr(self, expr: str | list) -> list:
        """
            ...
            2. json数组
                ['foo', 'myvar', "max([5,8,2,11, 7]),'a')", 'rand(100)+120', '3+4', "'singel;;quote'", '"double;;quote"', "`backquote;; ${bar}`"]
        """
        if isinstance(expr, str):
            ...
            raise TypeError(f"Expected str or list, got {type(expr).__name__}")
